...
def save_single_bundle(outdir: Path, emb: np.ndarray, ids: list, meta: list,
                       bot_id: str,
    ...
    if len(data_bytes) + approx_doc_overhead < (16 * 1024 * 1024):
        ...
        try:
            ...
            gridfs_id = fs.put(data_bytes, filename=f"{bundle_id}.npz", uploaded_at=datetime.utcnow())
            bundles.replace_one(
                {"_id": bundle_id},
                {
                    "_id": bundle_id,
...
def main_for_bot(bot_id: str):
    ...
    save_single_bundle(OUTDIR, emb_norm, ids, meta, bot_id=bot_id)
